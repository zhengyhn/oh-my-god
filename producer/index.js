...
function generateTextWithHtml(text, html) {
  let set = new Set();
  let result = "";
  let j = 0;
  let i = html.indexOf(text.substr(0, 1));
  for (; i < html.length; ++i) {
    if (html.substr(i, 10) === '<img src="') {
      const end = html.indexOf('"', i + 10);
      const url = html.substring(i + 10, end);
      ...
      i = end;
    } else if (html.substr(i, 1) === text.substr(j, 1)) {
      result += text.substr(j, 1);
      // console.log(result);
      ++j;
      while (text[j] === "\n") {
        ++j;
...
function generateTextWithHtml2(text, html) {
  ...
  for (let i = 0; i < html.length; ++i) {
    if (html.substring(i, i + 10) === '<img src="') {
      ...
      if (!set.has(fileName)) {
        result += htmlToText(html.substring(j, i)) + "image[" + url + "]";
        set.add(fileName);
      }
      i = html.indexOf(">", end);
      j = i + 1;
  ...
  result += htmlToText(html.substring(j));
  return result;
}
